...
class RegistrationManager {
	...
	def getPitchRegistration( int note, IPitchSequence pcSeq ) {
...
class SetCreator {
    ...
    def createDesign( int length, int memory, List inSets ) {
        ...
        PitchSequence pcSeq = new PitchSequence()
        ...
        while ( count < length ) {
            ...
            if ( candidates.size > 0) {
                ...
                println "No PCSet-related candidates found, choosing pitch ${pitch} from ${arbitrary}"
        ...
        println ( pcSeq.toString() )
        return pcSeq
    }
    	
    def getCandidates( IPitchSequence pSeq, int mem, inSets) {
        def candidates = []
        def pcSetToTest
    ...
    def getArbitraryCandidates( IPitchSequence inSeq, int memory, int length = 4 ) {
        def candidates = []
...
List sets = [ [0,1,2,6], [0,1,4,6], [0,1,3,7], [0,1,5,7], [0,3,7,11] ]
def pSeqList = []
(0..numSequences).each {
